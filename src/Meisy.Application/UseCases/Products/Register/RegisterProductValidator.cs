...
namespace Meisy.Application.UseCases.Products.Register
{
    public class RegisterProductValidator : AbstractValidator<RequestRegisterProductJson>
    {
        public RegisterProductValidator() {
            ...
            RuleFor(product => product.MeasurementUnit).IsInEnum().WithMessage(ResourceErrorMessages.INVALID_VALUE_FOR_MEASUREMENT_UNIT);
            ...
            RuleForEach(product => product.ProductInputs).SetValidator(new RegisterProductInputsValidator());


            RuleFor(product => product.CreatedAt).LessThanOrEqualTo(DateTime.UtcNow).WithMessage(ResourceErrorMessages.INVALID_DATE);
